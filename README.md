...
## 🚀 Features
- **Queue Manager (Enhanced in v9.2.4):** Build complex batches by mixing different presets. Merge multiple JSON queues together. Get a full report of space saved at the end of the batch.
...
### Queue Management
Mediaenc v9.2.4 introduces a powerful Queue Manager.
1.  **Build a Queue:** Select files and settings for a batch.
2.  **Add Files:** Use `[a] Add Files` to start a new wizard session and append files with different settings (e.g., process 5 movies with GPU preset, then add 2 movies with CPU preset in the same run).
3.  **Merge Queue:** Use `[m] Merge Queue` to drag-and-drop another JSON queue file and combine it with the current list.
4.  **Export/Import:** Save your work plan to a JSON file on your Desktop (`[e] Export`) and load it later (`[4] Import Queue` from main menu) to resume processing.
