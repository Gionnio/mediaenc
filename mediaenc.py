...
Description: Optimized for macOS Apple Silicon.
             Features: Smart Audio (EAC3), Robust Benchmark, Technical Preset Naming.
"""
...
def get_quality_verdict(score, metric="VMAF"):
    ...
    if metric == "VMAF":
        ...
        if s >= 0.99: return "ECCELLENTE", "Praticamente identico", Colors.GREEN
        if s >= 0.98: return "BUONO", "Fedeltà molto alta", Colors.CYAN
        if s >= 0.95: return "ACCETTABILE", "Buona fedeltà", Colors.WARNING
        return "SCARSO", "Differenze strutturali", Colors.FAIL
...
# ============================================================
def mode_test_bench():
    print(f"\n{Colors.HEADER}=== TEST BENCHMARK (Sample 45s - No Crop) ==={Colors.ENDC}")
    ...
    if not selected_presets: print("Nessun preset valido."); return
    ...
    for preset in selected_presets:
        ...
        # SIZE ESTIMATION (Video + Audio)
        video_size_gb = (out_file.stat().st_size / 45) * duration / (1024**3)
        audio_bitrate_str = preset.get('audio_bitrate', '320k')
        try:
            audio_kbps = int(audio_bitrate_str.replace('k', ''))
        except:
            audio_kbps = 320 
        
        # Audio GB = (kbps * seconds) / 8 / 1024 / 1024
    ...
    print(f"\n{Colors.HEADER}=== RISULTATI BENCHMARK COMPLETO (v8.8) ==={Colors.ENDC}")
    ...
    for r in results:
        ...
        print(f"{r['name']:<32} | {vmaf_str:<5} | {voto_str} | {ssim_val:<6} | {eff:.2f}  | {r['size_gb']:.1f} GB")
    print("-" * 90)
...
# ============================================================
def main():
    print(f"{Colors.BOLD}=== MEDIAENC – Ultimate Suite v8.8 (Official) ==={Colors.ENDC}")
    check_deps()
