...
def save_queue(jobs):
    print(f"\n{Colors.CYAN}--- EXPORT QUEUE ---{Colors.ENDC}")
    default_name = "mediaenc_queue.json"
    desktop_path = Path.home() / "Desktop"
    fname = input(f"Filename (Enter for '{default_name}'): ").strip()
    ...
    try:
        ...
        print(f"{Colors.GREEN}✔ Queue saved to: {full_path}{Colors.ENDC}")
    except Exception as e: print(f"{Colors.FAIL}Save error: {e}{Colors.ENDC}")
...
def load_queue():
    print(f"\n{Colors.CYAN}--- IMPORT QUEUE ---{Colors.ENDC}")
    print("Drag JSON queue file (q=Back):")
    raw_path = input("> ").strip()
    if raw_path.lower() == 'q': return None
    
    jobs_data = parse_json_queue(raw_path)
    if jobs_data:
        print(f"{Colors.GREEN}✔ Loaded {len(jobs_data)} jobs.{Colors.ENDC}")
    return jobs_data
...
def select_tracks(streams, track_type="audio"):
    ...
    selected = []
    defaults = [k for k, v in map_indices.items() if v['lang'] == 'eng'] if track_type == "audio" else []
...
def get_quality_verdict(score, metric="VMAF"):
    ...
    if metric == "VMAF":
        if s >= 95: return "EXCELLENT", "Indistinguishable", Colors.GREEN
        if s >= 93: return "GREAT", "Imperceptible", Colors.GREEN
        if s >= 90: return "GOOD", "High Quality", Colors.CYAN
        if s >= 80: return "ACCEPTABLE", "Visible", Colors.WARNING
...
def mode_test_bench():
    ...
    while True:
        ...
        if not ref_file.exists():
            print(f"{Colors.FAIL}Error creating ref.{Colors.ENDC}"); return
        ...
        for preset in selected_presets:
            ...
            if run_ffmpeg_piped(cmd, 45):
                ...
                if is_copy: vmaf, ssim = 100.0, 1.0
                    ...
                    print(f"   {Colors.BLUE}⚙️ Calculating metrics (VMAF/SSIM) - Please wait...{Colors.ENDC}")
                    ref_chain = "[1:v]"
        ...
        print(f"\n{Colors.HEADER}=== BENCHMARK RESULTS ==={Colors.ENDC}")
        ...
        print("\n[1] Retry | [2] Encode with Winner | [q] Quit")
        c = input("> ").strip()
        if c == '2':
             sel_p = input("Preset number: "); idx = int(sel_p)-1
...
def mode_quality_check():
    print(f"\n{Colors.HEADER}=== QUALITY CHECK ==={Colors.ENDC}")
    ...
    if opt_crop == 'y': force_no_crop = True
    ...
    print("\n3. Choose Metric (q=Back): [1] VMAF [2] SSIM")
    ...
    if metric_choice == "1":
        model = "vmaf_v0.6.1"
        if input("Model [1] 4K HDR [2] 1080p SDR: ").strip() == "1": model = "vmaf_4k_v0.6.1"
    ...
    print(f"\n{Colors.BLUE}🚀 Starting analysis...{Colors.ENDC}")
    ...
    print(f"\n{Colors.BOLD}RESULT: {score}{Colors.ENDC}")
...
def wizard_new_jobs(direct_file=None, direct_preset=None):
    ...
    if direct_preset:
        ...
        print(f"\n{Colors.CYAN}★ Preset selected: {current_preset['name']}{Colors.ENDC}")
    else:
    ...
    for idx, f in enumerate(files):
        print(f"\n{Colors.WARNING}=== Configuring {idx+1}/{len(files)}: {f.name} ==={Colors.ENDC}")
        ...
        if is_video_copy:
            crop_filter = None
            print(f"{Colors.BLUE}ℹ️ Remux Mode: Auto-Crop disabled.{Colors.ENDC}")
        else:
            if crop_filter:
                print(f"{Colors.GREEN}  → Crop detected: {crop_filter}{Colors.ENDC}")
                if input(f"  Confirm? [y/n] (Enter=Yes): ").lower() == 'n':
                    m = input("  Manual Crop or Enter for NO: ").strip()
                    crop_filter = f"crop={m}" if m and not m.startswith("crop=") else (m if m else None)
            else:
                m = input("  Manual Crop or Enter for original: ").strip()
                crop_filter = f"crop={m}" if m and not m.startswith("crop=") else (m if m else None)
            
        print("\nChoose Audio Strategy:")
        print(" [1] Passthrough (Copy 1:1)")
        print(f" [2] Smart Surround (EAC3 if needed)")
...
def run_job_execution(jobs):
    ...
    if len(jobs) > 1 and total_in > 0:
        print(f"\n{Colors.HEADER}=== BATCH SUMMARY ==={Colors.ENDC}")
        print(f"Files Processed:  {len(jobs)}")
        print(f"Total Original:   {format_size(total_in)}")
        print(f"Total Encoded:    {format_size(total_out)}")
        saved = total_in - total_out
        pct = (saved / total_in) * 100 if total_in > 0 else 0
        print(f"Total Saved:      {format_size(saved)} ({pct:.1f}%)")

def mode_queue_manager(jobs=[]):
    if not jobs: print("Queue empty."); return

    while True:
        print(f"\n{Colors.HEADER}=== QUEUE SUMMARY ({len(jobs)} Files) ==={Colors.ENDC}")
        for i, j in enumerate(jobs):
            print(f"{i+1}. {j['input'].name} -> {j['preset']['name']}")
            print(f"   Audio: {len(j['sel_audio'])} tracks | Mode: {j['audio_mode']}")
        print("-" * 40)
        print(f"{Colors.GREEN}[s] START (Execute Jobs){Colors.ENDC}")
        print(f"{Colors.CYAN}[a] ADD FILES (Wizard){Colors.ENDC}")
        print(f"{Colors.BLUE}[m] MERGE QUEUE (From JSON){Colors.ENDC}")
        print(f"{Colors.WARNING}[e] EXPORT (Save queue to file){Colors.ENDC}")
        print("[q] EXIT (Return to menu)")
        ...
        if choice == 's': run_job_execution(jobs); return
            ...
            if add_jobs:
                ...
                print(f"{Colors.GREEN}✔ Added {len(add_jobs)} new jobs.{Colors.ENDC}")
        elif choice == 'm':
            print("Drag JSON queue file to merge:")
            jpath = input("> ").strip()
            new_jobs = parse_json_queue(jpath)
            if new_jobs:
                jobs.extend(new_jobs)
                print(f"{Colors.GREEN}✔ Merged {len(new_jobs)} jobs.{Colors.ENDC}")
