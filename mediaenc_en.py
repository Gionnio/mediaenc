    ...
    for i, job in enumerate(jobs):
        ...
        if run_ffmpeg_piped(cmd, job['duration']):
            ...
            if Path(str(job['output'])).exists():
                ...
                print(f"{Colors.CYAN}📊 Ratio: {ratio:.1f}% (Saved: {format_size(diff)}){Colors.ENDC}")
...
def mode_queue_manager(jobs=[]):
    ...
    while True:
        ...
        print(f"{Colors.CYAN}[a] APPEND (Add more files/presets){Colors.ENDC}")
